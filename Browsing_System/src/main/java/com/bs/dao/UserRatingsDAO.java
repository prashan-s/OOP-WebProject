...
public class UserRatingsDAO implements IUserRatingsDAO {
	...
	public List<UserRatings> selectUserRatings(int rateId) {
		...
		try {
			...
			while (rs.next()) {
				...
				UserRatings userRating = new UserRatings(returnedRateId, userId, type, tvsId, movieId, rating,
						rowCreatedDatetime);
	...
	// select movie ratings for a given movie
	...
	// select movie ratings for a given tv series
	public List<UserRatings> selectTVSeriesRatings(int tvsId) {
		...
		try {
			...
			// Retrieve the counts from the result sets
			if (rs1.next()) {
				favoriteCount = rs1.getInt(1);
			}

			if (rs2.next()) {
				watchCount = rs2.getInt(1);
			}
